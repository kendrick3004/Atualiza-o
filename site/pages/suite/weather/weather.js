/**
 * ARQUIVO: weather.js
 * DESCRICAO: Motor de clima com geolocalizacao e suporte offline.
 * VERSAO: 2.6.0 - Chaves carregadas dinamicamente com fallback seguro
 */
...
const WeatherModule = (function() {
    ...
    function showErrorMessage(msg) {
        ...
        if (footer) {
            footer.innerHTML = `
                <div class="weather-content" style="display:flex; align-items:center; justify-content:center; height:100%; font-size:12px; color:rgba(255,255,255,0.6);">
                    <span>🌦️ ${msg}</span>
                </div>
            `;
        }
    ...
    function getCustomAssets(conditionCode, isDay) {
        ...
        return {
            iconPath: `/assets/dev/WEATHER/${iconFile}`,
    ...
    async function fetchWeather() {
        if (!CONFIG.API_KEY) {
            CONFIG.API_KEY = await ENV.getAsync('WEATHER_API_KEY');
        }

        if (!CONFIG.API_KEY) {
            console.warn('[Weather] Chave de API nao carregada. Usando dados locais.');
            const cachedData = loadFromLocal();
            if (cachedData) updateUI(cachedData);
            else showErrorMessage('Clima indisponivel no momento');
            return;
        }
    ...
    async function init() {
        ...
        // Aguardar o ENV estar pronto antes de buscar clima
        await fetchWeather();
